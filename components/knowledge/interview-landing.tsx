    ...
    <div className="flex-1 min-w-0 max-w-5xl">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
          {[
            ...
            <div key={s.label} className="glass-card p-4 text-center">
              <s.icon size={20} className="text-[#6D5DF6] mx-auto mb-2" />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
          {[
            { href: "/interview/companies", icon: Building2, title: "Companies", desc: `${stats.companiesDetected} company interview experiences`, color: "from-[#6D5DF6] to-[#8B5CF6]" },
            ...
            <Link key={card.href} href={card.href} className="glass-card p-5 hover:border-[#6D5DF6]/30 transition-colors group">
              ...
              <h3 className="font-bold group-hover:text-[#6D5DF6] transition-colors">{card.title}</h3>
          ...
          <section className="mb-10">
            ...
            <div className="space-y-2">
              {recent.slice(0, 5).map((a) => a && (
                <Link key={a.route} href={a.route} className="glass-card p-4 block hover:border-[#6D5DF6]/30">
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {bookmarks.slice(0, 8).map((r) => {
                ...
                return a ? <Link key={r} href={r} className="px-3 py-2 rounded-xl bg-[#6D5DF6]/10 text-[#6D5DF6] text-sm font-medium">{a.title}</Link> : null;
              })}
            </div>
          </section>
        )}
