...
export function KnowledgeNav() {
  ...
  return (
    <header className="sticky top-0 z-50 px-3 sm:px-4 pt-3 pb-2">
      <div className="max-w-[1400px] mx-auto flex items-center gap-2 sm:gap-3 h-14 px-3 sm:px-4 rounded-2xl bg-white/80 dark:bg-background/80 backdrop-blur-xl border border-border/80 dark:border-white/10 shadow-sm">
        ...
        <nav className="flex items-center gap-1 text-sm text-slate-400 min-w-0" aria-label="Breadcrumb">
          ...
          <Link href="/interview" className={cn("hover:text-brand transition-colors shrink-0 flex items-center gap-1", isHub && "text-brand font-medium")}>
        ...
        <Link href="/interview/search" className="text-sm text-slate-500 hover:text-brand hidden sm:block min-h-[44px] flex items-center">Search</Link>
