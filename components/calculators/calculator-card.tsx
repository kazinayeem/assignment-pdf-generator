  ...
  return (
    ...
    >
      <div className="glass-card h-full p-5 sm:p-6 flex flex-col group relative overflow-hidden">
        <div className="absolute -right-6 -top-6 w-24 h-24 rounded-full bg-brand/5 blur-2xl group-hover:bg-brand/10 transition-colors" aria-hidden />

        <div className="flex items-start justify-between mb-4 relative">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-brand to-brand-secondary flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform">
          ...
          {showFavorite && (
            ...
            >
              <Bookmark size={18} className={cn(fav ? "fill-brand text-brand" : "text-slate-400")} />
            </button>
          )}
        </div>

        <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white mb-2 group-hover:text-brand transition-colors">
          {calculator.title}
        </h3>
        ...
        {category && (
          <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-lg bg-brand/8 text-brand border border-brand/15 w-fit mb-4">
