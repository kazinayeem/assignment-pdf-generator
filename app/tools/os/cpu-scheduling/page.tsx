       ...
       <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
            <div>
              <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-gray-900">CPU Scheduling Simulator</h1>
              <p className="text-gray-500 text-xs sm:text-sm mt-1">Simulate FCFS, SJF, SRTF, Round Robin and Priority scheduling with Gantt chart.</p>
            </div>
            <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
              {[["Avg WT", avgWT + "ms"], ["Avg TAT", avgTAT + "ms"], ["CPU Util", cpuUtil + "%"]].map(([l, v]) => (
                <div key={l} className="flex-1 sm:flex-none px-2 sm:px-3 py-1.5 sm:py-2 bg-sky-50 rounded-xl border border-sky-100 text-center">
                  <p className="text-xs sm:text-sm font-black text-sky-600">{v}</p>
                  <p className="text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-sky-400">{l}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4">
            {(["FCFS", "SJF", "SRTF", "RR", "Priority"] as Algorithm[]).map((a) => (
              <button key={a} onClick={() => setAlgo(a)}
                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-sm font-bold border transition-all ${algo === a ? "bg-sky-600 text-white border-sky-600 shadow-sm" : "bg-white text-gray-600 border-gray-200 hover:border-sky-300"}`}>
                {a}
              </button>
            ))}
          </div>
          <div className="p-3 sm:p-4 bg-sky-50 rounded-xl border border-sky-100 flex flex-col sm:flex-row gap-3 sm:gap-4 items-start">
            <div className="flex-1 min-w-0">
              <p className="text-[11px] sm:text-xs font-black text-sky-700 mb-1">{algo} — {ALGO_INFO[algo].preemptive ? "Preemptive" : "Non-Preemptive"}</p>
              <p className="text-[11px] sm:text-xs text-sky-600 leading-relaxed">{ALGO_INFO[algo].desc}</p>
            </div>
            <div className="text-[11px] sm:text-xs text-sky-500 shrink-0">Best for: <span className="font-bold">{ALGO_INFO[algo].best}</span></div>
          </div>
          {algo === "RR" && (
            <div className="mt-3 flex items-center gap-2 sm:gap-3">
              <label className="text-[11px] sm:text-xs font-bold text-gray-500">Time Quantum:</label>
              <input type="number" min={1} max={20} value={quantum} onChange={(e) => setQuantum(parseInt(e.target.value) || 1)}
                className="w-16 sm:w-20 h-8 sm:h-9 px-2 sm:px-3 text-xs sm:text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-sky-400" />
              <span className="text-[11px] sm:text-xs text-gray-400">ms</span>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={addProcess} className="flex items-center gap-1.5 text-[11px] sm:text-xs font-bold text-sky-600 hover:text-sky-700 transition-colors">
              <Plus className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Add Process
            </button>
          </div>
                    ...
                    <td className="py-2 px-2 sm:px-3">
                      <div className="flex items-center gap-1.5 sm:gap-2">
                        ...
                        <input value={p.name} onChange={(e) => updateProcess(p.id, "name", e.target.value)}
                          className="w-12 sm:w-16 h-7 sm:h-8 px-1.5 sm:px-2 text-[11px] sm:text-sm font-bold rounded-lg border border-gray-200 focus:outline-none focus:border-sky-400" />
                      </div>
                    </td>
                    <td className="py-2 px-2 sm:px-3">
                      <input type="number" min={0} value={p.arrival} onChange={(e) => updateProcess(p.id, "arrival", e.target.value)}
                        className="w-14 sm:w-20 h-7 sm:h-8 px-1.5 sm:px-2 text-[11px] sm:text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-sky-400" />
                    </td>
                    <td className="py-2 px-2 sm:px-3">
          ...
          <div className="flex gap-2 sm:gap-3 mt-4">
            <button onClick={run} className="flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 sm:py-2.5 bg-sky-600 hover:bg-sky-700 text-white text-xs sm:text-sm font-bold rounded-xl transition-colors shadow-sm">
              <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Run Simulation
            </button>
            <button onClick={reset} className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs sm:text-sm font-bold rounded-xl transition-colors">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
            ...
            <div className="overflow-x-auto pb-2">
              <div className="flex items-end gap-0 min-w-max">
                {gantt.map((block, i) => {
                  const width = Math.max(36, ((block.end - block.start) / totalTime) * 600);
                  return (
                    <div key={i} className="flex flex-col items-center">
                      <div className="flex items-center justify-center text-white text-[10px] sm:text-xs font-black rounded-sm"
                        style={{ width, height: 36, backgroundColor: block.color, minWidth: 36 }}>
                        {block.name}
                      </div>
                      <span className="text-[8px] sm:text-[9px] text-gray-400 mt-1">{block.start}</span>
                    </div>
                  );
                })}
                <div className="flex flex-col items-center">
                  <div style={{ width: 2, height: 36, backgroundColor: "transparent" }} />
                  <span className="text-[8px] sm:text-[9px] text-gray-400 mt-1">{totalTime}</span>
                </div>
              </div>
            </div>
          </div>
                    ...
                    <tr key={r.name} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      <td className="py-2 px-2 sm:px-3">
                        <div className="flex items-center gap-2">
                          <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full shrink-0" style={{ backgroundColor: COLORS[i % COLORS.length] }} />
                          <span className="font-bold text-xs sm:text-sm">{r.name}</span>
                        </div>
                      </td>
                      <td className="py-2 px-2 sm:px-3 text-gray-600 text-xs sm:text-sm">{r.arrival}</td>
                      <td className="py-2 px-2 sm:px-3 text-gray-600 text-xs sm:text-sm">{r.burst}</td>
                      <td className="py-2 px-2 sm:px-3 font-bold text-sky-600 text-xs sm:text-sm">{r.completion}</td>
                      <td className="py-2 px-2 sm:px-3 font-bold text-violet-600 text-xs sm:text-sm">{r.turnaround}</td>
                      <td className="py-2 px-2 sm:px-3 font-bold text-emerald-600 text-xs sm:text-sm">{r.waiting}</td>
                    </tr>
                  ))}
                  <tr className="bg-gray-50 font-bold">
                    <td className="py-2 px-2 sm:px-3 text-gray-700 text-xs sm:text-sm" colSpan={4}>Averages</td>
                    <td className="py-2 px-2 sm:px-3 text-violet-700 text-xs sm:text-sm">{avgTAT}</td>
                    <td className="py-2 px-2 sm:px-3 text-emerald-700 text-xs sm:text-sm">{avgWT}</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
          <h2 className="text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 mb-4 flex items-center gap-2">
            <Info className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-sky-500" /> Algorithm Comparison
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-xs sm:text-sm">
              <thead>
              ...
              <tbody>
                {[
                  ...
                  <tr key={name} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${name === algo ? "bg-sky-50" : ""}`}>
                    <td className="py-2 px-2 sm:px-3 font-bold text-gray-800 text-xs sm:text-sm">{name}</td>
                    <td className="py-2 px-2 sm:px-3"><span className={`text-[10px] sm:text-xs font-bold px-1.5 sm:px-2 py-0.5 rounded-full ${pre === "Yes" ? "bg-orange-50 text-orange-600" : "bg-gray-100 text-gray-500"}`}>{pre}</span></td>
                    <td className="py-2 px-2 sm:px-3"><span className={`text-[10px] sm:text-xs font-bold px-1.5 sm:px-2 py-0.5 rounded-full ${starv === "Yes" ? "bg-red-50 text-red-500" : "bg-emerald-50 text-emerald-600"}`}>{starv}</span></td>
                    <td className="py-2 px-2 sm:px-3 text-gray-600 text-[10px] sm:text-xs">{over}</td>
                    <td className="py-2 px-2 sm:px-3 text-gray-500 text-[10px] sm:text-xs">{best}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Viva */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
          <h2 className="text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 mb-4">Interview Questions</h2>
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="p-3 sm:p-4 bg-sky-50 rounded-xl border border-sky-100">
                <p className="text-xs sm:text-sm font-bold text-gray-800 mb-1">Q: {qa.q}</p>
                <p className="text-xs sm:text-sm text-sky-700">A: {qa.a}</p>
              </div>
            ))}
          </div>
        </div>
